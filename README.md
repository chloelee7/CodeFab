...
**라인 편집 / 히스토리**: 실제 터미널에서 REPL·디버그 모드를 실행하면 JLine 라인 편집기가 활성화되어, `↑`/`↓` 화살표로 이전·이후 입력을 불러오고(세션 내 히스토리) 한글 등 멀티바이트 입력의 백스페이스 잔상 없이 편집할 수 있습니다. 파이프·파일·테스트 같은 비대화형 입력에서는 자동으로 기존 줄 단위 입력 경로로 폴백합니다.
...
```bash
./factory selfhost run hello.cfab
```

하위 호환 별칭으로 `selfhost <file>`도 동작합니다.

```bash
./factory selfhost hello.cfab
```
...
| 명령어 | 설명 |
|--------|------|
| `step` | 다음 문장에서 정지 (함수·블록 내부로 진입, step-into) |
| `next` | 현재 문장 실행 후 같은/상위 레벨 다음 문장에서 정지 (내부 진입 안 함, step-over) |
