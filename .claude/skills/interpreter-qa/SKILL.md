...
| **상태 수명 × 최적화 (REPL 교차-run)** | 한 run에서 만들어 globals에 저장되는 값(함수 클로저 등)이 보유한 AST 노드 + 그 노드의 해석정보(distance) | 후속 run에서 그 저장된 값을 실행할 때 Executor가 쓰는 locals/Environment | run마다 locals 맵을 **교체**해 이전 run 함수 본문 노드의 distance가 사라짐 → 본문 지역변수가 전역 폴백되어 `Undefined variable` (실제 발생 버그). 누적(putAll)해야 함 |

## 상태 수명(state lifetime) 경계 — REPL 교차-run

성능 최적화(정적 바인딩, 폴딩)나 값 모델(클로저)이 **여러 run에 걸쳐 사는 상태**와 만나는 지점을 반드시 점검한다. 한 run에서 만든 AST 노드·해석정보가 다음 run에서 실행되는 값(특히 globals에 저장된 함수) 안에 살아남아야 한다.

- **핵심 질문**: "run N에서 정의해 globals에 저장한 함수를 run N+M에서 호출하면, 그 본문의 지역변수/파라미터가 여전히 올바르게 해석되는가?" — distance 맵이 run마다 교체되면 깨진다(누적이 정답).
- **반드시 있어야 할 회귀 테스트**: REPL에서 (a) 한 입력으로 함수 정의 → 다른 입력에서 호출, (b) 재귀 함수도 동일. 단발 실행은 한 run에 모든 노드가 들어가 이 버그를 못 잡으므로, **세션(교차-run) 테스트가 따로 있어야 한다.**
- 일반화: "이 데이터의 수명(run 1회 vs 세션 전체)이 그것을 참조하는 다른 데이터의 수명과 일치하는가?" 불일치(짧은 수명 맵 ↔ 긴 수명 함수값)가 곧 버그다.
