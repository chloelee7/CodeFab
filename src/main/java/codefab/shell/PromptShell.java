...
/**
 * Interactive REPL. It reads input line by line and accumulates a multi-line
 * buffer until the input looks complete — that is, all parentheses and braces
 * are balanced and the input ends in a way that can stand on its own (a
 * semicolon or a closing brace). The buffer is then run through a single
 * persistent {@link CodeFabSession} so variables survive between inputs.
 */
public final class PromptShell {
    ...
    public void run() {
        ...
        try {
            while (true) {
                ...
                if (line == null) break; // EOF (e.g. piped input or Ctrl-D)
    ...
    public static boolean isComplete(String source) {
        ...
        if (inString || parens > 0 || braces > 0) return false;
